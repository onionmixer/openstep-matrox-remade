#!/bin/sh
# Keep the P3 software reference oracle independent of hardware interfaces.

set -eu

script_dir=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
source_root=$(CDPATH= cd -- "$script_dir/../reference" && pwd)
sanitized=$(mktemp "${TMPDIR:-/tmp}/openstep-mga-reference.XXXXXX")
...
' "$source_root/OpenStepMGAReference.c" "$source_root/OpenStepMGAReference.h" > "$sanitized"
...
if grep -n -E '^[^:]+:[0-9]+:#[[:space:]]*(include|import)[[:space:]]*<|(IO(Map|Physical|PCI|FrameBuffer|GetDisplay|SetDisplay)|IODirectDevice|task_self|port_(allocate|deallocate)|kern_serv_|inb|inw|inl|outb|outw|outl|pciConfig(Read|Write)|pci_config_(read|write)|MGA_(PCI|MMIO))' "$sanitized"; then
    echo "OPENSTEP_MGA_REFERENCE_STATIC_GUARD_STATUS=fail" >&2
    exit 1
fi
echo "OPENSTEP_MGA_REFERENCE_STATIC_GUARD_STATUS=pass"
