...
2. 혼합쌍(MM2S/MM8S = GL *_MIPMAP_LINEAR)은 **미개방 유지**: f 가
   λ_hw(가수-선형)·1/16 내림이라 Mesa 대비 최대 G 4코드(±2 문턱 초과).
   결정적·단조·모델화 가능한 편차이므로, 성능이 필요하면 "문서화된
   근사"로 개방하는 선택지가 남아 있다 — 별도 결정 사항.
