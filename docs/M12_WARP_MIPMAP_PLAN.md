...
### 10-1. 결정 실험 실측 (2026-08-31, 3차) — 확정

```
m2-1.375: G 40   m2-1.625: G 48    -> MM2S 는 혼합기 (MM8S 와 동일 λ 산술)
m1-1.625: G 64                     -> MM1S 는 레벨 반올림 (λ_hw 1.542 → L2)
```

**확정 명명 (실측 정본, 스펙 9612-9614 이 뒤집힘 — §6-4 는 역전이었다)**:
| 엔진 | 실측 동작 | GL 대응 |
|---|---|---|
| MM1S | 레벨 반올림, 무혼합, 포인트 | NEAREST_MIPMAP_NEAREST |
| MM2S | 2탭: 레벨당 1탭 혼합 | NEAREST_MIPMAP_LINEAR |
| MM4S | 레벨 반올림, 무혼합, bilinear | LINEAR_MIPMAP_NEAREST |
| MM8S | 8탭: 레벨당 4탭 혼합 (trilinear) | LINEAR_MIPMAP_LINEAR |

**판정**:
1. 프로덕션 교정 — GL_LINEAR_MIPMAP_NEAREST 를 MM2S→**MM4S** 로 (이번
   커밋).  admission 허용쌍도 {MM1S, MM4S}.  레벨 선택은 반올림으로
   Mesa 와 같은 방향이며, 경계 위치만 λ_hw 근사만큼 이동(무해).
2. 혼합쌍(MM2S/MM8S = GL *_MIPMAP_LINEAR)은 **문서화된 근사로 개방**
   (2026-08-31 결정): f 가 λ_hw(가수-선형)·1/16 내림이라 Mesa 대비 최대
   G 4코드(f 오차 1/8) — 결정적·단조이며 이 표가 편차의 정본이다.
   NEAREST_MM_LINEAR→MM2S, LINEAR_MM_LINEAR→MM8S, admission 은 네 모드
   전부.  정확 조건(§7 의 MaxLevel 고정 등)은 네 모드 공통.
3. Phase A §6-4 의 교훈: 정수 λ 관측만으로 혼합기와 선택기를 구별할 수
   없다 (혼합이 정수점에서 퇴화).  분수점 실측이 정본.
