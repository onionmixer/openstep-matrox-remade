...
기준일: 2026-08-19
상태: **완료(2026-08-19). D1-0 ~ D1-2 전부 실기 PASS.** 결과는 §7.
아래 본문은 착수 전 계획(codex 교차검토 후 개정판)이며, 초판의 결함 5건은
§1-6에 적혀 있다.
...
**G450은 시스템 메모리에서 명령 목록을 PCI 버스마스터로 가져와 실행한다.**
S5 §6이 물은 질문의 답이며, 3D 스택을 만들 근거가 생겼다.

| 단계 | 결과 |
| --- | --- |
| D1-0 | `IOMallocLow` 64 KiB → `phys=0x20000`, 64 KiB 정렬, conventional memory 안, 8 KiB 페이지 7회 프로브 전부 연속 |
| D1-1 | 6블록 조립, 16개 레지스터 쓰기가 손 디코딩과 전부 일치. `OPMODE` 거부(`index=-1`) |
| D1-2 | 음성 대조 통과 → `PRIMADDRESS 0 → 0x20064`(발행 tail과 일치) → 픽셀 불일치 **0**, checksum `dbeef000` |

checksum `0xDBEEF000`은 S1이 MMIO로 낸 것과 **같은 값**이며, 로그를 믿지 않고
`0xDEADBEEF × 4096 mod 2^32`로 독립 재계산해 대조했다.

부수 확인: 가상주소 = 물리주소(`0x20000`)라 conventional memory는 커널에서
아이덴티티 매핑돼 있다. 그럼에도 코드는 `IOPhysicalFromVirtual`을 거친다 —
우연히 같은 것에 기대지 않는다.

### 7-1. 완료 판정을 DRM에서 그대로 옮기면 안 된다

첫 실행은 `spins=100000` 타임아웃으로 끝났다. **하드웨어는 정상이었다.**
그때의 `STATUS=0x80820025`를 풀면:

| 비트 | 의미 | 값 |
| --- | --- | --- |
| bit17 `ENDPRDMASTS` | primary DMA가 끝에 도달 | 1 |
| bit16 `DWGENGSTS` | 드로잉 엔진 busy | 0 |
| bit0 `SOFTRAPEN` | 링 끝의 SOFTRAP 발화 | **1** |

즉 전송은 이미 끝나 있었다. 실패한 것은 폴링 조건이다. DRM을 따라
`(status & ENGINE_IDLE_MASK) == ENDPRDMASTS`를 기다렸는데, 이 비교는
`SOFTRAPEN`이 **0**이기를 요구한다 — 그런데 그 비트가 켜지는 것이 바로
완료 신호다. **자기 완료 신호가 자기 대기 조건을 영구히 막는 구조**였다.

DRM에서 그 비교가 성립하는 이유는 인터럽트 핸들러가 `ICLEAR`로 트랩을
지우기 때문이다. 폴링 설계에는 그 단계가 없으므로 트랩이 래치된 채 남는다.
**코드를 옮길 때 그 코드가 서 있던 문맥(인터럽트 구동)을 함께 옮기지 않은
전형적인 실수다.**

고친 조건은 세 가지 동시 성립이다: 트랩 발화 **AND** 드로잉 엔진 idle
**AND** DMA 끝 도달. 확인 후 `ICLEAR`로 트랩을 소비한다. 같은 하드웨어가
`spins=22`에 완료를 보고했다.

### 7-2. 봉쇄 수단은 설계대로 작동했다

- 조립기 화이트리스트가 `OPMODE`를 거부했고(D1-1), 같은 이유로 링에는
  `PRIMADDRESS`/`PRIMEND`를 인코딩할 인덱스가 존재하지 않는다 — 링이 자기를
  읽는 엔진을 재프로그래밍할 수 없다.
- 음성 대조가 먼저 돌았고 통과했다. 이것이 없었다면 양성 결과를 "MMIO로
  이미 써둔 것"과 구분할 수 없었다.
- 버퍼 해제는 규칙대로 두 조건(읽기 포인터가 tail 도달 + 엔진 idle)이 모두
  성립했을 때만 이뤄졌다. 첫 실행에서는 실패 분기로 빠져 보유했다.

## 8. 다음

`S5_HW3D_DMA_FEASIBILITY.md` §5의 6개 항목 중 **1번(PCI 버스마스터 DMA 링)이
끝났다.** 남은 것은 WARP 마이크로코드 업로드, 3D 상태·정점 스트림 생성기,
DMA 완료 동기화(인터럽트화), VRAM 텍스처 관리, Mesa 하드웨어 드라이버 신규
작성이다. 규모가 다르므로 각각 별도 계획서와 교차검토가 필요하다.


---

## 정정 (2026-08-19) — 화이트리스트가 막지 못하던 넷

이 문서와 코드 주석은 "group0/group1 규칙상 DMA 주소 레지스터는 인코딩될
수 없다"고 적었다. **`PRIMADDRESS`(0x1e58)와 `PRIMEND`(0x1e5c)에 대해서만
참이다** — 그 둘은 두 그룹 밖에 있다.

**부차 링과 셋업 링의 주소 레지스터는 group1 안에 있다**: `SECADDRESS`
0x2c40, `SECEND` 0x2c44, `SETUPADDRESS` 0x2cd0, `SETUPEND` 0x2cd4
(`mga_drv.h:500-503`). group1은 0x2c00~0x2dff이므로(`:228-229`) 규칙이
넷을 전부 통과시켰다.

노출은 없었다 — 커널이 유일한 호출자이고 그 넷을 지목한 적이 없다.
그래도 `osmgaDmaIndex`가 넷을 명시적으로 거부하도록 고쳤다. 정본 설명은
`M1_MESA_BRIDGE_PLAN.md` §4-4.

**회귀 확인 미완**: D1 경로가 config에서 꺼져 있는 동안 고쳤으므로, 다시
켤 때 링 시험이 통과하는지 확인해야 한다(`REMAINING_WORK.md` §3-7).
