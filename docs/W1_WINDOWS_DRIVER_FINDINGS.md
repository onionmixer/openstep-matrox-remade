...
# 2차 분석 시도와 그 결과 (2026-08-19)

## X-1. codex 2차 시도 — 타임아웃

`DWGCTL` 레지스터 경유 쓰기 11곳을 역추적하는 과제를 넘겼으나 **1800초
무응답으로 중단**됐다. 1차(12개 질문)도 같은 이유로 실패했다.

**패턴**: 성공한 것은 좁은 과제(질문 4개, 이미 좁혀진 한 군집)였고, 실패한
둘은 넓은 과제였다. 이 도구는 **한 번에 하나의 좁은 질문**에만 쓸 수 있다.

## X-2. 데이터 스캔 시도 — **방법이 무효라 폐기**

코드 추적이 막혀서, `DWGCTL`로 그럴듯한 값을 바이너리 전체에서 찾는 방법을
시도했다. 첫 스캔은 110개 고유값을 뱉었는데 `0xff0c2474`처럼 상위 바이트가
`DWGCTL`일 수 없는 것들이 섞여 있어 대부분 노이즈였다.

조건을 조이고 **대조군을 넣었더니 방법 자체가 무효로 드러났다**:
...
이미 아는 값 셋 중 **하나도 4바이트 정렬로 존재하지 않는다.** 이 값들은
정렬된 상수 테이블이 아니라 **코드 스트림 안의 즉시값으로만** 있다. 따라서
정렬 데이터 스캔은 이 질문에 답할 수 없고, 그 결과는 전부 버린다.

**대조군을 넣지 않았다면 8개 후보를 발견으로 보고했을 것이다.**

## X-3. 그래서 아직 모르는 것

**"이 HAL이 `ARZERO`/`SGNZERO`가 지워진 `DWGCTL`을 내는가"** — 즉 기울어진
변을 DWG 엔진으로 걷는가 — 는 **여전히 미확정**이다. 답하려면 레지스터 경유
11곳의 역추적이 필요하고, 그것은 이 도구로 한 번에 되지 않는다.

## X-4. 판단 — 이 경로의 수확 체감

Windows 드라이버에서 실제로 얻은 것은 다음과 같고, 적지 않다:

| 얻은 것 | 값어치 |
| --- | --- |
| `%ecx`가 MMIO 구경임을 증명(FIFO 대기 루프가 우리 것과 동형) | 이후 모든 분석의 토대 |
| `AR3`가 BITBLT 소스 주소라는 **세 번째 독립 확인** | 미해결 항목 하나를 닫음 |
| `0x000c7076`(TEXTURE_TRAP\|I)이 우리 `0x000c7074`와 opcode 한 비트 차 | D3 값 선택이 출하 계열과 같음을 뒷받침 |
| 즉시값 `DWGCTL` 전수조사(13곳 7값) | 상수 경로에는 기울어진 그리기가 **없다** |
| HAL은 BITBLT에 CLIPDIS를 켠다 | 하드웨어 클립이 공짜가 아닐 수 있다는 신호 |

반면 **기울어진 변 셋업은 X.Org XAA에 이미 완전한 참고 구현이 있다**
(`mgaSubsequentSolidFillTrap`, D3-2 §1). 남은 미지수는 오차항 `e`의 정확한
의미 하나이고, **D3-2의 검증 방법(행별 첫/마지막 x)이 `e = 0`이 맞는지를
한 번의 부팅으로 보여준다.**

→ **Windows 바이너리 추가 분석보다 D3-2 실기 실험이 싸다.** 분석은 여기서
멈추고, `e`가 틀린 것으로 드러나면 그때 11곳 역추적으로 돌아온다.

---

# 3차 — 11곳 전수 조사 (6배치, 2026-08-19)

과제를 작은 단위로 쪼개니 **전부 완료됐다.** 넓은 과제 두 번은 1800초
타임아웃으로 죽었고, 2~3곳씩 나눈 여섯 배치는 모두 빠르게 끝났다.
원본 보고는 `W1_RUN3A.md`~`W1_RUN3F.md`.

## Y-1. 전수 결과

| 사이트 | 베이스 | 결과 |
| --- | --- | --- |
| `baa156ca` | **구조체** | 레지스터 쓰기가 아님 — 값이 `[base+0x64] * ecx` 곱셈 |
| `baa184db` | **구조체** | 같은 `imul [base+0x64]` 패턴 |
| `baa1ab4d` | **구조체** | 같음 |
| `baa25f1d` | **구조체** | 같음 |
| `baa28836` | **구조체** | 같음 |
| `baa41e3a` | MMIO | BITBLT (`0x84004018` + 런타임) |
| `baa42c7c` | MMIO | ILOAD 계열 |
| `baa4942b` | MMIO | `\| 0x800c7804` → **배제** |
| `baa49739` | MMIO | `\| 0x800c7804` → **배제** |
| `baa542c4` | MMIO | `\| 0x800c7804` → **배제** |
| `baa559cb` | MMIO | `\| 0x800c7804` → **배제** |

## Y-2. 배제 논증 (직접 확인함)

```
baa4972b:  mov 0x2fa(%eax),%edx
baa49731:  and %edi,%edx
baa49733:  or  $0x800c7804,%edx
baa49739:  mov %edx,0x1d00(%ecx)
```

`0x800c7804`는 **ARZERO(bit12)와 SGNZERO(bit13)가 켜져 있고 `OR`는 비트를
지울 수 없다.** 런타임 값이 무엇이든 이 사이트는 축 정렬만 낸다. 네 사이트가
같은 구조다. 런타임 값을 모르고도 **증명**이 되는 형태라는 점이 좋다.

덧붙여 `0x800c7804`는 **우리 S1 단색 채우기 값 `0x000c7804`에 CLIPDIS를
더한 것**이다. HAL의 표준 solid fill이 우리 값과 같다.

## Y-3. 결론 — **질문 16이 참이다**

**`G400DD32.dll`은 DWG 엔진으로 기울어진 변을 그리지 않는다.**

- 즉시값 13곳: BITBLT/ILOAD, 그리고 `TEXTURE_TRAP|I` 하나(ARZERO/SGNZERO 켜짐)
- 레지스터 경유 11곳: 5곳은 레지스터 쓰기가 아니고, 2곳은 BITBLT/ILOAD,
  4곳은 OR 상수로 배제

→ **출하 Windows 드라이버는 삼각형 셋업을 전부 WARP에 맡긴다.**

## Y-4. 우리 계획에 주는 의미

D3 계획서는 "경로 B(래스터라이저 직접 구동)가 `mga_storm.c`라는 우리와 같은
조건의 참고 구현을 갖는다"고 적었다. **그 참고는 여전히 유효하지만, 3D
드라이버의 참고는 아니라는 것이 이제 확정됐다** — Windows 3D 드라이버조차
그 길을 쓰지 않는다.
...
| | 상태 |
| --- | --- |
| 기울어진 변을 DWG로 걷는 것 자체 | X.Org XAA에 **완전한 참고 구현 있음**(2D 다각형 채우기용) |
| 그것을 3D 삼각형에 쓴 선례 | **없음** — Windows HAL도 X.Org DRI도 WARP를 쓴다 |
| Mesa 3.4.2와의 정합 | 여전히 경로 B가 맞음(소프트웨어 T&L, 삼각형 단위 훅) |

즉 **D3-2는 여전히 할 수 있고 XAA 레시피가 있지만, 우리가 걷는 길에
3D 선례가 없다는 것을 알고 걷는 것**이다. 그 사실 자체가 이번 분석의
가장 큰 소득이다.

## Y-5. 방법에 대하여

- **작게 쪼개니 됐다.** 넓은 과제 2회 실패(각 1800초), 좁은 배치 6회 전부 성공.
- codex가 `OR`로는 비트를 못 지운다는 **증명**을 찾아낸 것은 좋은 작업이다.
  런타임 값을 모르는 채로 결론을 낼 수 있게 해줬다.
- 내가 과제 문서에 적은 기준이 틀렸었다(§2B-1): "지금까지 모든 값이
  ARZERO/SGNZERO 켜짐"은 **BITBLT 계열에는 거짓**이다. BITBLT는 `AR`로
  소스를 걷기 때문에 원래 꺼져 있다. 배치 중간에 발견해 정정했다.
