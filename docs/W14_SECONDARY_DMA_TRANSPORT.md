...
## 13. 1~5 단계 결과 (2026-08-28 실기)

### 13.1 부팅 로그가 셋을 답한다

```
W14: ring contiguous over 65536 bytes from 20000 (8 pages)
W14: secondary region +61456, 4080 bytes, 204 DMAPAD packets
W14: PRIMPTR fffffbf0 -> 0 (primptren 00 -> 00)
```

**모든 수치가 계산과 일치한다** (python 대조):

| | 로그 | 계산 |
| --- | --- | --- |
| 링 연속 구간 | 65,536 B | 65,536 |
| 2 차 구역 오프셋 | 61,456 | `24576 + 36880` |
| 2 차 구역 크기 | 4,080 B | 204 × 20 |
| DMAPAD 패킷 | 204 | 4080 / 20 |

**★ 링은 실제로 물리적으로 연속이었다.** 여덟 번의 페이지 확인이 전부 통과했고,
그것이 **이 전제가 확인된 첫 번째 순간**이다 — 봉쇄 논거 전체가 여기 기대고
있었는데 증거는 다른 할당에 대한 것이었다.

부수 사실: 8 페이지로 65,536 B 이므로 **이 커널의 `PAGE_SIZE` 는 8,192 B** 다.
`RING_OFFSET` 24,576 은 그 3 배라 클라이언트 매핑 경계가 페이지에 맞는다.

**★ `PRIMPTR` 의 `fffffbf0` 이 사라졌다.** enables 는 원래 `00` 이었고 그대로
`00` 이다 — 바뀐 것은 **포인터가 결정적이 됐다**는 것이고, 그것이 요점이다.

### 13.2 목록 용량을 4,080 B 줄였는데 회귀가 그대로다

```
15/15 통과 (tpq · ttex · tdc · tsa 포함)
```

거부 코드는 **17 · 6 · 13 · 5** 넷이 한 번씩 — 전부 시험이 **일부러** 유발하는
기존 검사(`E_TRICROSS`, `E_TEXORG`, `E_TEXCOORD`, `E_ZORG`)다.
**정렬 코드(20/21/22)도, 용량 부족으로 인한 새 거부도 없다.**

인코더 최악 32,640 B 에 대해 목록이 36,880 B 이므로 4,240 B 여유가 남고,
**실측이 그 계산을 확인해 준다.**

### 13.3 스냅샷

```
SNAP post-w14  version=2
  SECADDRESS.2c40  00000000
  SECEND.2c44      00000000
```

`secActive`·`secUnknown` 이 둘 다 내려가 있으므로 **정상적으로 읽는다.**
버전이 2 로 올라간 것이 보인다 — 마커가 나올 수 있는 스냅샷임을 옛 독자가
알 수 있어야 한다.

### 13.4 아직 안 한 것

**V6·V7 은 하지 않았다.** 네 번의 교차검토가 모두 그 실험 설계를 거부했고,
지금 상태는 **토대만 검증된 것**이다. 다음에 V7 을 설계할 때 바뀐 것:

- 링이 연속임을 **안다** (가정이 아니라)
- `PRIMPTR` 이 0 임을 **안다**
- 2 차 구역이 `DMAPAD` 로 채워져 있음을 **안다**
- 범위 검사기가 양쪽 경계에서 시험돼 있다
- 관측치가 정해져 있다 — `DWGSYNC` 매직값 (§12.1)
