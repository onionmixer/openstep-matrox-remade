...
## 11. 재부팅 2 회차 — 여섯 조합이 전부 확인됐다

```
Aug 29 17:35:47  OpenStepMGA C2: WARP 3D preference is Yes
```

teapot 의 제출 수로 층을 읽는다(WARP 128, 사다리꼴 193):

```
              cap 0 (체크박스 끔)      cap 1 (체크박스 켬)
env 없음      193  사다리꼴            128  WARP
env=1         128  WARP                128  WARP
env=0         193  사다리꼴            193  사다리꼴
```

**대각선 두 칸이 이 설계의 전부다.**  `cap 1 · env 없음` 이 128 이라는 것은
**환경변수 없이 체크박스만으로 층이 바뀐다**는 뜻이고, `cap 1 · env=0` 이
193 이라는 것은 **환경변수가 config 를 이긴다**는 뜻이다 — 이 저장소의 모든
측정이 그 성질에 기대고 있다.
...
패널 자체는 자동으로 확인할 수 없다.  연결되지 않은 outlet 은 nil 이고
nil 에 보낸 메시지는 **아무 말도 하지 않으므로**, 체크박스가 보이지 않거나
보이는데 아무것도 안 쓰는 실패는 로그에 남지 않는다.  Configure.app 을
띄워 두었고, 확인할 것은 셋이다:

```
Display 드라이버 인스펙터에 세 번째 스위치가 보이나
...
## 13. 체크박스를 켠 채 부팅 — 스위트가 처음으로 WARP 기본으로 돌았다

```
Aug 29 17:44:34  OpenStepMGA C2: WARP 3D preference is Yes
teapot, 환경변수 없음: 128 제출, 카드가 100%
```

그리고 **빠른 회귀 전항목 통과.**  이것이 새로운 이유는 이 스위트가 지금까지
언제나 사다리꼴을 기본으로 돌았기 때문이다 — `-warp` 스윕 변종이 따로 있는
것이 그 증거다.  이번에는 기계 설정 하나로 **모든 고정물이 WARP 을 지나갔다.**

추론이 아니라 측정으로 확인했다.  고정물 중 넷이 WARP 개수를 보고하는데,
층을 **고정하는 것은 하나도 없다**:

```
기본(체크박스 켬)      cov: warp=1     회귀 통과
OSMGA_MESA_WARP=0      cov: warp=0     회귀 통과
```

### 그리고 뒤집으면 이렇다

기계가 WARP 을 기본으로 잡으면 **사다리꼴 층이 기본 회귀에서 빠진다.**
그것을 되찾는 것이 정확히 `env=0` 의 새 의미이고, 위 두 줄이 그 팔도
여전히 깨끗함을 보인다.  `M17` 이 두 층을 다 살려 두기로 한 이상, 어느
쪽이 기계 기본값이든 **양쪽을 다 돌릴 수 있어야** 한다.
