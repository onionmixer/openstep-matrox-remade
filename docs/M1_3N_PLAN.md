...
## 11. 고친 뒤 (2026-08-21) — 다섯 전부 통과

| 경우 | 고치기 전 | 고친 뒤 |
| --- | --- | --- |
| 1 행 길이 320→333 | 여전히 가속, 그림 어긋남 | **통과** (`drew=0 declined=0`, 그림 맞음) |
| 2 320x120 재바인딩 | 여전히 가속 | **통과** (`surface=0`, 소프트웨어) |
| 3 두 번째 컨텍스트 | 3 가지 실패 + 프로세스 죽음 | **통과** — B 는 자기 버퍼에 그리고, B 파괴가 A 의 표면을 안 가져가고(`surface=4194304`), A 가 돌아와 가속으로 그린다. 안 죽는다 |
| 4 `ReleaseBuffer(NULL)` | 통과 | **통과** (유지됨) |
| 5 `Y_UP` 끄기 | 여전히 가속 | **통과** |

회귀 없음: `zagree` 320x240 과 640x480 이 이전과 **글자 그대로 같고**
(`lt=1035/1120, worst=2, mix1NotMin=0, mix2NotMin=0, declined=0`),
`zsize` 도 그대로다(800x600·320x240 하드웨어 깊이, 333x222 생성에서 거절).

### 구현하며 측정으로 잡은 것 둘 — 둘 다 설계에 없었다

**(가) 알파를 되돌리면 죽는다.** 설계는 "껐던 것과 같은 조건으로 다시 켠다"
였다. 켜자 **첫 `glClear` 에서 버스 에러**가 났다. 소프트웨어 알파 버퍼는
버퍼 크기가 정해질 때 할당되는데, 그 시점에는 우리 치환이 이미 플래그를
꺼둔 뒤다 — **한 번도 할당된 적이 없다.** 플래그만 켜면 클리어가 없는 곳을
가리킨다.

되돌리지 않는 것이 안전할 뿐 아니라 옳다: 알파를 끈 이유가 "알파는 화소의
네 번째 바이트에 산다"였고, 그건 이제 그릴 애플리케이션 버퍼에도 그대로
참이다.

**(나) 상태를 직접 갱신하는 것만으로는 삼각형 함수가 안 바뀐다.**
`osmesa_update_state` 를 직접 불렀는데도 다음 그리기에서 **우리 삼각형
함수가 호출됐고**, 커널이 배치를 거절해 그 프로세스의 가속이 revoke 됐다.
`gl_update_state` 는 드라이버에게 묻기 **전에** `Driver.TriangleFunc` 을
`NULL` 로 놓는데(`state.c:1100`), 직접 호출은 그 단계를 건너뛴다.

그래서 둘 다 한다 — 직접 호출은 **미러 래퍼를 지금 떼기 위해**(다음 그리기
전에 `glFinish` 가 오면 해제된 표면을 복사한다), `NewState |= NEW_ALL` 은
**삼각형 함수가 제대로 다시 선택되게** 하기 위해.

시험을 먼저 만들지 않았다면 (가)는 크래시로, (나)는 조용한 revoke 로
남았을 것이다.
