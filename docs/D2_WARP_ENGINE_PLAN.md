...
기준일: 2026-08-19
상태: **D2-0a / D2-0b 실기 PASS(2026-08-19).** 결과는 §7.
D2-1 이후는 미착수. 초판의 안전 논리는 교차검토로 뒤집혔다 — §3-0 참조.
...
**이 G450에 WARP 엔진이 있고 설정을 받아들인다.**

| 단계 | 결과 |
| --- | --- |
| D2-0a | 진입 시 `WMISC=0 WIADDR2=0`(WARP는 부팅 직후 꺼져 있다). `WMISC` 0x0B 쓰고 **0x3 되읽음** → PASS |
| D2-0b | 전체 G400 시퀀스 후 `WMISC=3`, `WIADDR2=0` → PASS. **설정된 채 suspend** = D2-1의 전제 상태 |
| 회귀 | 같은 부팅에서 D1-2 재통과(`spins=23`, checksum `dbeef000`), 화면 무변화 |

`0x0B`를 썼는데 `0x3`이 읽히는 것은 정상이며, 이는 우리가 만든 기준이 아니라
DRM 자신의 판정(`WMISC_EXPECTED`)이다.

### 7-1. stale 복원을 안 한 판단에 대하여

진입 시 두 레지스터가 모두 0이었으므로, **이번에는** 복원했어도 무해했을
것이다. 그렇다고 §3-0의 판단이 불필요했던 것은 아니다 — 그 값은 **미리 알
수 없었고**, `WIADDR2`가 `|3`이었다면 복원이 곧 임의 주소에서의 파이프
실행이었다. 사후에 안전했다고 사전 판단이 과했던 것으로 읽지 않는다.

### 7-2. 아직 증명되지 않은 것

- `WGETMSB`/`WVRTXSZ`/`WACCEPTSEQ`가 **실제로 그 값을 받았는지**는 모른다.
  소스 어디에도 되읽는 코드가 없어 검증 수단이 없다. 없는 검증을 지어내지
  않았고, 이 셋이 틀렸다면 그 증상은 D2-2(파이프 실행)에서야 드러난다.
- 마이크로코드 없이 `warp_init`을 한 순서 이탈(§1-4)이 결과에 영향을 주지
  않았다는 것도 **증명된 것이 아니라** `WMISC` 판정이 통과했을 뿐이다.

## 8. 다음

D2-1(마이크로코드 업로드)의 전제가 갖춰졌다. S5 §5의 6개 중 1번은 D1에서
끝났고, 2번의 관문이 여기서 열렸다.
