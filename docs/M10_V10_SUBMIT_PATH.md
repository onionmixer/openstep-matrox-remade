...
## 13. 세 번째 실행 (2026-08-29 09:22) — **다섯 전부 통과, 그림까지**

```
1  v9 before   verdict 0, dwords 20                    ✅
2  v10 dry     verdict 0, dwords 134                   ✅
3  v10 dry x4  verdict 25 / 26 / 7 / 4, dwords 0       ✅
4  v10 live    verdict 0, dwords 134
   되읽기      drawn 1176 of 1176, wrong 0,
               outside touched 0                        ✅
5  v9 after    verdict 0, dwords 20                    ✅
```

**커널이 클라이언트가 준 버전 10 배치를 받아 정확한 그림을 그린다.**

되읽기를 붙인 이유는 판정만으로는 부족하기 때문이다 — `verdict 0` 은 제출이
끝났다고 말하지 **화소가 움직였다고 말하지 않는다.**  목적지를 매핑해
sentinel 로 칠하고, 그린 뒤 하네스가 사다리꼴 경로와 두 번 합의한 모양과
대조한다: 다리 48 의 직각삼각형, 1176 화소, 그리고 **밖은 하나도 안 건드림**.

### 13.1 2 단계에서 실기가 가르친 것 둘

```
마이크로코드   생산 경로가 시험이 돌았기를 전제하고 있었다
warp_init      생산 경로가 하지 않았다 -- 검증도 인코딩도 멀쩡한데
               엔진이 목록을 실행하지 않았다
```

**둘 다 다섯 시험 행렬이 아니었으면 "엔진 실패" 로 보였을 것이다.**
dry 넷이 엔진을 건드리기 전에 멈추는 것을 보여줬고, v9 대조군이 경로 자체는
멀쩡함을 보여줬다.

### 13.2 남은 것

```
3 단계  훅 분기 -- WARP pending, 거절 세 단위, 층 전환 플러시
4 단계  혼합 층 A/B, 오프셋 시험, 시간 측정
```
