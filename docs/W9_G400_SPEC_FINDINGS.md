...
§9 는 이 문서를 처음 쓸 때의 목록이었고 **1~3 은 그 뒤에 끝났다** — §5.3 은
닫혔고, Q3(WARP 정지)·Q4(봉쇄)는 §6·§7 에 들어왔고, W6·W7·W8 의 뒤집힌 결론은
정정됐다(W8 은 통째로 철회). **남은 것은 조사가 아니라 판정과 설계다.**

1. **W2 §7 의 `SECADDRESS` 금지를 다시 판정한다.** 금지의 전제는 "회수 수단이
   없다" 였고 §5 가 그것을 무너뜨렸다. **다만 전제가 무너진 것이 금지가 틀렸다는
   뜻은 아니다** — §3-62 의 프리즈 일곱 번은 여전히 설명되지 않았다.
2. **W9 §10.2 의 넷을 1 차 경로에서 먼저 실증한다**: abort 검출
   (`DEVCTRL.recmastab`/`rectargab`), `RST.softreset` 회수, `WIADDRNB2` = 0 WARP
   정지, 배치마다 `clipdis` = 0. **2 차 DMA 를 켜기 전에.**
3. **`CACHEFLUSH` 로 정착 읽기를 대체할지는 실측 후에 정한다**(§9.5.3).
