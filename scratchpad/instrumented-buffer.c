...
{
    fprintf(stderr, "OSMGA-CLAIM released\n");
    /*
                        ...
                        int *rowLength)
{
    ...
    if (width <= 0 || height <= 0 || rowLength == 0)
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 22\n", 22); return 0; } while (0);
    /*
     ...
     */
    if (appRowLength <= 0)
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 30\n", 30); return 0; } while (0);
    /*
    ...
    if ((unsigned long)appRowLength < (unsigned long)width)
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 44\n", 44); return 0; } while (0);
    ...
    OSMGAMesaProbeRun(&probe);
    if (probe.verdict != OSMGA_PROBE_HARDWARE)
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 93\n", 93); return 0; } while (0);
    ...
    if (bufMapped != 0) {
         ...
         */
        if (bufCtx != ctx)
            do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 104\n", 104); return 0; } while (0);
        if (bufWidth == (unsigned long)width &&
            ...
            *rowLength = (int)bufStride;
            ...
            do { fprintf(stderr, "OSMGA-CLAIM TOOK IT at body line 118\n", 118); return bufMapped; } while (0);
        }
        /*
         ...
         * slow.
         */
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 126\n", 126); return 0; } while (0);
    }
    ...
    if (rshift != 16 || gshift != 8 || bshift != 0)
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 138\n", 138); return 0; } while (0);
    ...
    if (stride == 0UL || (unsigned long)width > stride)
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 152\n", 152); return 0;               /* a row would run into the next */ } while (0);
    if (stride > probe.caps[OSMGA_HW3D_CAP_STRIDE])
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 154\n", 154); return 0;               /* wider than the pitch register is known to hold */ } while (0);
    /*
    ...
    if (stride > probe.caps[OSMGA_HW3D_CAP_STRIDE])
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 170\n", 170); return 0; } while (0);
    ...
    if ((unsigned long)height > avail / (stride * 4UL))
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 178\n", 178); return 0; } while (0);
    need = (unsigned long)height * stride * 4UL;

    if (vm_allocate(task_self(), &addr, (vm_size_t)need, TRUE) != KERN_SUCCESS)
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 182\n", 182); return 0; } while (0);
    if ((int)mmap((caddr_t)addr, (int)need, PROT_READ | PROT_WRITE,
                  MAP_SHARED, OSMGAMesaProbeDeviceFd(),
                  (long)probe.caps[OSMGA_HW3D_CAP_VRAMOFF]) == -1) {
        (void)vm_deallocate(task_self(), addr, (vm_size_t)need);
        do { fprintf(stderr, "OSMGA-CLAIM REFUSED at body line 187\n", 187); return 0; } while (0);
    }
    ...
    *rowLength = (int)stride;
    osmgaMesaBufferImport();
    do { fprintf(stderr, "OSMGA-CLAIM TOOK IT at body line 220\n", 220); return bufMapped; } while (0);
}
