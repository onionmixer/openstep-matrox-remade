...
/*
 * M12 -- the WARP-path mip-fetch qualification atlas.  Three levels laid
 * out at independent 32-byte-aligned origins (spec Table 4-2: with
 * offsetselect zero, TEXORG..TEXORG4 are five absolute map origins), a
 * guard band after them, and every texel carrying its level in the alpha
 * byte and its own offset in R:G with (R*R+G*G)&255 in B -- M1-4D9's
 * mix-resistant signature.  docs/M12_WARP_MIPMAP_PLAN.md.
 */
#define OSMGA_M12_TEX        (15UL * 1024UL * 1024UL)
#define OSMGA_M12_L0_DIM     64UL
#define OSMGA_M12_L1_OFF     16384UL
#define OSMGA_M12_L2_OFF     20480UL
#define OSMGA_M12_GUARD_OFF  21504UL
#define OSMGA_M12_BYTES      22528UL
#define OSMGA_M12_F32_TWO    0x40000000UL   /* 2.0f  = 0.5 * rhw 4 */
#define OSMGA_M12_MAPNB2     (2UL << 29)    /* three maps: 64/32/16 */

/*
 * The one hook the production list builder has for M12: when this is not
 * null the texture block is emitted with these five ABSOLUTE level
 * origins, the NONLINEAR power-of-two pitch the old DRI uses for mip
 * chains (tpitch = log2(width)-3, no PITCHLIN), and the caller's raw
 * TEXFILTER word.  Production never sets it; the band sets it under the
 * engine claim and clears it on every exit.  A static instead of a
 ...
 */
/*
 * Which bands the next qualification run emits.  See the parameter
 * handler: the log is the scarce resource, not the engine time.
 */
#define OSMGA_WARPQUAL_ALL      1UL
                   ...
                   count:(unsigned)count
{
    ...
    if (osmgaTextEquals(parameterName, OSMGA_WARPQUAL_PARAM)) {
        ...
        /*
         * The value selects which bands run, because the log cannot hold
         * them all.  syslogd drops bursts -- it dropped T0 through T7b of
         * the 2026-08-29 run -- and the repaired harness is about eighty
         * lines where fifty was already too many.
         *
         *   1 (or absent)  every band, as before
         *   2              T0..T6 only -- the settled ones
         *   3              T7 onward   -- the ones under repair
         *
         * Splitting is what the boot-path comment beside runRaster... has
        ...
        if (parameterArray != 0 && count == 1U) {
            if (parameterArray[0] == 2U)
                osmgaWarpQualBands = OSMGA_WARPQUAL_SETTLED;
...
- (void)enterLinearMode
{
    ...
    /*
     * The WARP qualification is NOT run here any more.
     *
     * It runs twice a boot otherwise -- once from this line and once from
     * the OSMGAWarpQual parameter -- and each run is about fifty lines, so
     * syslogd drops one of the two bursts.  It dropped T0 through T7b on
     * 2026-08-29 and the measurements went with them; only the SUMMARY line
     * survived because it was put there deliberately.
     *
     * The comment three lines below already said this about D3-3b and left
     * two other tests out for the same reason.  The trigger exists exactly
     * so the harness can run once, with the system up and syslogd draining:
     * the run that used it kept all 25 lines.
     *
     * To run it: cc the probe in test/openstep-mga-warpqual-probe.m and
     * poke OSMGAWarpQual.  docs/NEXT_BOOT.md has the procedure.
     */
    /* [self runWarpDepthQualificationTest]; */
...
typedef struct {
    ...
    /*
     * TEXCTL's clamp bits, which T7 has to be able to CLEAR per axis.
     * They were a literal here until repeat needed testing; the driver
     * records at :655 that CLAMPU is 0x10000000 and CLAMPV 0x08000000,
     * measured rather than guessed.
     */
    unsigned long clamp;
    /*
     * TDUALSTAGE0 and TDUALSTAGE1, which take the SAME value.  Selecting
     * the diffuse alpha in stage nought alone fixes exactly half the
     * pixels -- the even columns take it and the odd ones go on taking the
     * texture's, measured as `20 ab 28 ab 30 ab 38 ab` (:13249).  One
...
/*
 * Mesa's own initial ALPHACTRL: src=ONE dst=ZERO, atmode noacmp, aten
 * disabled.  Both alpha tests off (W17) and nothing blends, which is what
 * every band before T8 wants.  It was a literal inside the list builder
 * until T8 needed to change it.
 */
#define OSMGA_M3_ALPHACTRL_NOBLEND  0x00000001UL
                ...
                const char *label)
{
    ...
    if (!osmgaDmaBuildTriangleList(ring, ringDwords, pipePhys, stride,
                                   ...
                                   dwgctl, alphactrl, dstorg, zorg, tex,
                                   &tailDwords, &totalDwords))
        return 0;
                 ...
                 unsigned long alphactrl)
{
    ...
    /*
     * FCOL is written exactly as the caller gave it, alpha included.  It
     * used to be masked to RGB, which is invisible while nothing blends
     * and wrong the moment something does: T8b's oracle needs a source
     * alpha to blend WITH, and a masked one would have made the two paths
     * differ for a reason that has nothing to do with WARP.  T0 and T6
     * pass the same value they always did, masked at the call.
     */
    osmgaW32(base, MGA_FCOL,      colour);
    osmgaW32(base, MGA_ALPHACTRL, alphactrl);
...
/*
 * ---- T7: does the texture unit wrap the same way when WARP drives it? ----
 *
 * GL_REPEAT is NOT a new question for this driver.  M1_4CB_REPEAT.md ran
 * it: section 8-1 confirmed on a rebooted kernel which CLAMPUV bit is
 * which axis, section 8-2 measured the LINEAR seams (u 127 / v 128, corner
 * 127) and opened repeat for bilinear as well, and the encoder has
 * required pitch == width and a power-of-two axis ever since (:13128).
 *
 * So T7 asks the one thing that is new -- whether coordinates produced by
 * WARP wrap like coordinates produced by the trapezoid setup -- and
 * borrows the earlier verdicts to compare against rather than re-deriving
 * them.
 *
 * The map is a power of two with pitch == width, which repeat requires:
 * spec 3-211 says tpitchext must be a power of two when either clamp bit
 * is clear, and a masked index into a padded surface addresses the wrong
 * row.
 */
#define OSMGA_M4_REP_BASE     40960UL      /* 40960 % 32 == 0 */
#define OSMGA_M4_REP_PITCH       16UL
#define OSMGA_M4_REP_ROWS        16UL
#define OSMGA_M4_REP_DIM         16UL
#define OSMGA_M4_REP_LOG2         4UL

/*
 * ONE TEXEL A PIXEL, which is what the first attempt got wrong.
 *
 * It swept four texels a pixel to fit three periods of a 64 texel map into
 * 48 pixels, and at that rate half a pixel of anchor moves the sample two
 * texels -- the width of a bin.  The band could not then separate "the
 * hardware wrapped differently" from "the anchor is not where I assumed",
 * and it did not: the CLAMP CONTROL disagreed on 360 of 2304 pixels, which
 * is the expected map being wrong rather than the hardware.
 *
 * A sixteen texel map gives three periods in 48 pixels at exactly one
 * texel a pixel, which is T3's rate -- and T3 got 1176 of 1176 exact on
 * three of its four phases there.  Half a pixel now moves half a texel.
 *
 *      texel(dx) = -16 + dx        dx = 0 .. 47
 *
 * wraps three times and disagrees with the clamped map on 30 of the 48
 * columns (python), so the two are still far apart.
 */
#define OSMGA_M4_REP_STEP         1L
#define OSMGA_M4_REP_START     (-16L)
#define OSMGA_M4_REP_HOLD     0x3F080000UL   /* 8.5/16 -- the held axis */
#define OSMGA_M4_REP_HOLD_TEXEL   8L

/*
 * FOUR PHASES, for T3's reason.  Its phase 0 sat on a texel boundary and
 * was wrong on 1175 of 1176 pixels while the other three were exact; a
 * single phase would have called the anchor correct or broken depending
 * on which one it happened to be.  Each phase adds a quarter texel, which
 * is 1/64 in this map's coordinates.
 */
#define OSMGA_M4_REP_PHASES       4UL
static const unsigned long osmgaM4RepULo[OSMGA_M4_REP_PHASES] = {
    0xBF800000UL,   /* -1.000000 */
    0xBF7C0000UL,   /* -0.984375 */
    0xBF780000UL,   /* -0.968750 */
    0xBF740000UL    /* -0.953125 */
};
static const unsigned long osmgaM4RepUHi[OSMGA_M4_REP_PHASES] = {
    0x40000000UL,   /*  2.000000 */
    0x40010000UL,   /*  2.015625 */
    0x40020000UL,   /*  2.031250 */
    0x40030000UL    /*  2.046875 */
};

/*
 * The seam map is its own allocation so that T7e's 0/128/255 pattern and
 * T7a-c's address-coded texels do not have to share one, and refilling
 * between bands does not leave texture-cache staleness as a competing
 * explanation.
 */
#define OSMGA_M4_SEAM_BASE    57344UL      /* 57344 % 32 == 0 */
#define OSMGA_M4_SEAM_PITCH      16UL
#define OSMGA_M4_SEAM_ROWS       16UL
#define OSMGA_M4_SEAM_DIM        16UL
#define OSMGA_M4_SEAM_LOG2        4UL

/*
 * T8a's map, at its own address for the same reason: a fence proves the
 * DRAWING finished, not that the texture cache forgot, and "the old alpha
 * came back" and "the selector is wrong" would look the same.
 */
#define OSMGA_M4_A8_BASE      58368UL      /* 58368 % 32 == 0 */
#define OSMGA_M4_A8_PITCH        16UL
#define OSMGA_M4_A8_ROWS         16UL
#define OSMGA_M4_A8_DIM          16UL
#define OSMGA_M4_A8_LOG2          4UL

#define OSMGA_M4_TEX_BYTES    (OSMGA_M4_A8_BASE + OSMGA_M4_A8_PITCH * \
                               OSMGA_M4_A8_ROWS * 4UL)

/*
 * T7e's sweep: two periods of the sixteen texel map across the square, so
 * that u = 0 lands on dx = 3 and u = 1 on dx = 27 -- both PIXEL CENTRES,
 * which is where a seam can actually be read.  u = (dx - 3)/24.
 */
#define OSMGA_M4_SEAM_LO      0xBE000000UL   /* -3/24 = -0.125  */
#define OSMGA_M4_SEAM_HI      0x3FF00000UL   /* 45/24 =  1.875  */
#define OSMGA_M4_SEAM_NEAR       3UL         /* dx where u == 0 */
#define OSMGA_M4_SEAM_FAR       27UL         /* dx where u == 1 */

/*
 * ---- T8: alpha and blending ----
 *
 * OSMGA_MESA_BLEND_OVER, the value the shipping Mesa path sends for
 * GL_SRC_ALPHA / GL_ONE_MINUS_SRC_ALPHA (OpenStepMGAMesaTriangle.h:114).
 * Its blend arithmetic is already qualified against Mesa -- nine factor
 * pairs, four channels, worst difference nought
 * (M1_4E3_BLENDFACTOR_PLAN.md) -- so T8 asks only whether WARP reaches the
 * same picture, not whether the picture is right.
 */
#define OSMGA_M4_ALPHACTRL_OVER  0x01000154UL

/* The texture's own alpha in T8a, chosen so that "the interpolated alpha
 * arrived" and "the texture's alpha arrived" cannot look alike. */
#define OSMGA_M4_TEX_ALPHA       0x33UL

/* The destination T8b blends into.  Not black: blending against nought is
 * indistinguishable from not blending for the ONE/ZERO factors. */
#define OSMGA_M4_PREFILL      0x00204060UL
/* Half-transparent, so SRC_ALPHA / 1-SRC_ALPHA lands between the prefill
 * and the source and neither "did not draw" nor "did not blend" can be
 * mistaken for success. */
#define OSMGA_M4_SRC_COLOUR   0x80FF8040UL

/*
 * ALPHACTRL with alphamode = '01', "alpha channel".
 *
 * Spec 3-33: bits <9:8> select the alpha mode WRITTEN TO THE FRAME BUFFER,
 * and '00' is FCOL.  The harness's other bands use 0x0001, which is '00',
 * and the state list writes FCOL as nought -- so the destination's alpha
 * byte was FCOL's nought no matter what the texture or the vertex carried.
 * That is why T8a read 0..0 in BOTH of its bands on 2026-08-29: the
 * selector it was testing never had a chance to matter.
 *
 * Only T8a uses this.  Changing it everywhere would not move any verdict
 * -- every other band masks the alpha off -- but it would make the
 * oracle's stored alpha and WARP's differ where nobody is looking, and a
 * difference nobody observes is still a difference to explain later.
 *
 * srcblendf ONE and dstblendf ZERO are unchanged, which the spec calls
 * blending disabled in those words, so the colour is untouched.
 */
#define OSMGA_M4_ALPHACTRL_ACHAN  0x00000101UL

/*
 * T9's geometry.  Run A's triangles are SCATTERED along a diagonal, which
 * the first version did not do: it put all twenty at one position, so the
 * engine drew 210 pixels of work twenty times over the same addresses.
 * The helper still reported drawing in flight at the pointer boundary,
 * but D2-2f's threshold was measured with triangles that moved, and a
 * band whose precondition is "the engine is still busy" should be as far
 * past that threshold as it can cheaply be.
 *
 * Run A stays clear of run B's rows entirely, so "which run wrote this
 * depth" has one answer per pixel rather than a rule about ordering.
 */
#define OSMGA_M4_T9_TRIS        20UL
#define OSMGA_M4_T9_LEG         10UL
#define OSMGA_M4_T9_AROW        30UL     /* .. 30 + 19 + 10 = 59, the clip */
#define OSMGA_M4_T9_ACOL         6UL
#define OSMGA_M4_T9_BROW         8UL     /* clear of run A's rows */
#define OSMGA_M4_T9_BCOL        34UL
#define OSMGA_M4_T9_BLEG        20UL
...
osmgaM4BuildTri(unsigned long *v, const unsigned long *xs,
                ...
                const unsigned long *tv, const unsigned long *colour)
{
    unsigned long i, p = 0UL;
...
osmgaM4SquareVerts(unsigned long *v, unsigned long lo, unsigned long hi)
{
    ...
    unsigned long xs[3], ys[3], z[3], rhw[3], tu[3], tv[3], col[3];
    unsigned long i;
    ...
    col[0] = col[1] = col[2] = OSMGA_M3_COLOUR;
    osmgaM4BuildTri(v, xs, ys, z, rhw, tu, tv, col);

    /* (HI,LO) (HI,HI) (LO,HI) */
    xs[0] = OSMGA_M4_SQ_HI; ys[0] = OSMGA_M4_SQ_LO; tu[0] = hi; tv[0] = lo;
    xs[1] = OSMGA_M4_SQ_HI; ys[1] = OSMGA_M4_SQ_HI; tu[1] = hi; tv[1] = hi;
    xs[2] = OSMGA_M4_SQ_LO; ys[2] = OSMGA_M4_SQ_HI; tu[2] = lo; tv[2] = hi;
    osmgaM4BuildTri(v + OSMGA_D2C_VTX_DWORDS, xs, ys, z, rhw, tu, tv, col);
}
...
/*
 * A plain address-coded map with a chosen alpha.  No halo: T7 wraps rather
 * than reaching outside, and T8a wants the texture's own alpha to be a
 * value the interpolated one could never be mistaken for.
 */
static void
osmgaM4FillPlain(volatile unsigned long *tex, unsigned long pitch,
                 unsigned long rows, unsigned long alpha)
{
    unsigned long x, y;

    for (y = 0UL; y < rows; y++)
        for (x = 0UL; x < pitch; x++)
            tex[y * pitch + x] = (alpha << 24) | (x << 16) | (y << 8);
}

/*
 * The seam map, in M1_4CB section 8-2's shape: the first texel of an axis
 * nought, the last 255 and everything between 128.  At a seam a wrapping
 * filter blends 255 with 0 and reads about 127; one that clamped the
 * neighbouring tap instead reads 0 at the near seam and 255 at the far
 * one, which is how the two are told apart.  Red carries the u axis and
 * green the v axis, so one draw measures both.
 */
static void
osmgaM4FillSeam(volatile unsigned long *tex, unsigned long pitch,
                unsigned long rows, unsigned long dim)
{
    unsigned long x, y;

    for (y = 0UL; y < rows; y++)
        for (x = 0UL; x < pitch; x++) {
            unsigned long r = (x == 0UL) ? 0UL
                            : (x == dim - 1UL) ? 255UL : 128UL;
            unsigned long g = (y == 0UL) ? 0UL
                            : (y == dim - 1UL) ? 255UL : 128UL;
            tex[y * pitch + x] = (r << 16) | (g << 8);
        }
}

/*
 * T5's square with the two axes given independently, which T7 needs: one
 * axis sweeps out of range while the other is held safely inside, so a
 * band that wrapped u while leaving v clamped cannot pass for both.
 */
static void
osmgaM4SquareVertsUV(unsigned long *v, unsigned long uLo, unsigned long uHi,
                     unsigned long vLo, unsigned long vHi,
                     const unsigned long *corner)
{
    unsigned long xs[3], ys[3], z[3], rhw[3], tu[3], tv[3], col[3];
    unsigned long i;
    ...
    for (i = 0UL; i < 3UL; i++) {
        z[i]   = OSMGA_M3_F32_HALF;
        rhw[i] = OSMGA_M4_F32_ONE;
    }

    /* corner[] is (LO,LO) (HI,LO) (LO,HI) (HI,HI) -- T8a ramps the alpha
     * across it, so the two triangles must agree on the shared edge. */
    xs[0] = OSMGA_M4_SQ_LO; ys[0] = OSMGA_M4_SQ_LO; tu[0] = uLo; tv[0] = vLo;
    xs[1] = OSMGA_M4_SQ_HI; ys[1] = OSMGA_M4_SQ_LO; tu[1] = uHi; tv[1] = vLo;
    xs[2] = OSMGA_M4_SQ_LO; ys[2] = OSMGA_M4_SQ_HI; tu[2] = uLo; tv[2] = vHi;
    col[0] = corner[0]; col[1] = corner[1]; col[2] = corner[2];
    osmgaM4BuildTri(v, xs, ys, z, rhw, tu, tv, col);

    xs[0] = OSMGA_M4_SQ_HI; ys[0] = OSMGA_M4_SQ_LO; tu[0] = uHi; tv[0] = vLo;
    xs[1] = OSMGA_M4_SQ_HI; ys[1] = OSMGA_M4_SQ_HI; tu[1] = uHi; tv[1] = vHi;
    xs[2] = OSMGA_M4_SQ_LO; ys[2] = OSMGA_M4_SQ_HI; tu[2] = uLo; tv[2] = vHi;
    col[0] = corner[1]; col[1] = corner[3]; col[2] = corner[2];
    osmgaM4BuildTri(v + OSMGA_D2C_VTX_DWORDS, xs, ys, z, rhw, tu, tv, col);
}

/* -62 + 4*dx reduced the two ways.  The mask form rather than % because
 * C89 leaves the sign of a negative remainder to the implementation, and
 * the smallest value here is -62. */
/* The map is OSMGA_M4_REP_DIM wide; the mask form rather than % because
 * C89 leaves the sign of a negative remainder to the implementation and
 * the smallest value here is -16. */
static long
osmgaM4WrapTexel(long t)
{
    return (long)(((unsigned long)(t + 640L)) & (OSMGA_M4_REP_DIM - 1UL));
}

static long
osmgaM4ClampTexel(long t)
{
    return (t < 0L) ? 0L
                    : ((t > (long)(OSMGA_M4_REP_DIM - 1UL))
                           ? (long)(OSMGA_M4_REP_DIM - 1UL) : t);
}

/*
 * T7's verdict, per axis, and it does NOT ask for an exact texel.
 *
 * The sweep is four texels a pixel, so a sub-pixel difference in where
 * WARP anchors the coordinate moves the answer by up to two texels -- and
 * this is not the test that measures the anchor (T3 is).  What separates
 * wrapping from clamping is the SHAPE:
 *
 *   wrapped   (read - unwrapped) & 63 is the SAME value at every pixel,
 *             whatever the anchor offset happens to be
 *   clamped   that difference is not constant, because clamping is not a
 *             shift; the clamped map is matched exactly instead
 *
 * So the tallest histogram bin against the drawn count is the verdict for
 * a repeat band, and the exact clamp match is the verdict for a clamp
 * band.  `sweepU`/`sweepV` say which axis is moving; the held axis must
 * read its one texel, which is what makes a band that wrapped the wrong
 * axis fail rather than pass.
 */
static unsigned long
osmgaM4ReportWrap(volatile unsigned long *blk, unsigned long stride,
                  unsigned long lo, unsigned long hi,
                  int sweepU, int wrapU, int sweepV, int wrapV,
                  const char *label, int verbose)
{
    unsigned long row, col, seen = 0UL, undrawn = 0UL;
    unsigned long histo[64], k, top = 0UL, topN = 0UL;   /* DIM <= 64 */
    unsigned long exactClamp = 0UL, heldBad = 0UL;
    int sweptWrap = sweepU ? wrapU : wrapV;

    for (k = 0UL; k < OSMGA_M4_REP_DIM; k++)
        histo[k] = 0UL;

    for (row = lo; row < hi; row++) {
        for (col = lo; col < hi; col++) {
            unsigned long v = blk[row * stride + col] & OSMGA_D2E_RGB;
            long tx, ty, t, swept, held;
            ...
            swept = sweepU ? tx : ty;
            held  = sweepU ? ty : tx;
            t = OSMGA_M4_REP_START + OSMGA_M4_REP_STEP *
                (long)((sweepU ? col : row) - lo);

            histo[((unsigned long)(swept - t + 640L)) &
                  (OSMGA_M4_REP_DIM - 1UL)]++;
            if (swept == osmgaM4ClampTexel(t))
                exactClamp++;
            if (held != OSMGA_M4_REP_HOLD_TEXEL)
                heldBad++;
        }
    }
    for (k = 0UL; k < OSMGA_M4_REP_DIM; k++)
        if (histo[k] > topN) { topN = histo[k]; top = k; }

    {
        unsigned long bad = (heldBad != 0UL)
                                ? (heldBad + seen)
                                : (sweptWrap ? (seen - topN)
                                             : (seen - exactClamp));

        /*
         * Quiet when it agrees.  Twelve phases across three axes would
         * otherwise be twenty-four lines, and syslogd drops bursts -- it
         * dropped T0 through T7b of the last run.  A phase that agrees
         * says nothing the per-axis summary does not.
         */
        if (verbose || bad != 0UL) {
            IOLog("OpenStepMGA M4/%s: %lu px (%lu never drawn), held axis "
                  "wrong on %lu; tallest wrap bin %lu holds %lu, exact "
                  "clamp %lu\n",
                  label, seen, undrawn, heldBad, top, topN, exactClamp);
            osmgaD2Settle();
        }
        return bad;
    }
...
/*
 * T7e.  Not a pass or a fail: the two seams and the corner are printed so
 * they can be put beside M1_4CB section 8-2's trapezoid numbers (u 127,
 * v 128, corner 127).  A wrapping filter reads about half way; one that
 * clamps the neighbouring tap reads 0 at the near seam and 255 at the far
 * one, and no arithmetic is needed to tell those apart.
 */
static void
                  ...
                  unsigned long lo, const char *label, unsigned long *out)
{
    ...
    unsigned long hold  = lo + 12UL;          /* mid-texel on the other axis */
    unsigned long a, b, c, d, e;

    a = blk[hold  * stride + nearC] & OSMGA_D2E_RGB;
    b = blk[hold  * stride + farC ] & OSMGA_D2E_RGB;
    c = blk[nearC * stride + hold ] & OSMGA_D2E_RGB;
    ...
    IOLog("OpenStepMGA M4/%s: u near %lu far %lu | v near %lu far %lu | "
          ...
          (c >>  8) & 0xFFUL, (d >>  8) & 0xFFUL,
          (e >> 16) & 0xFFUL, (e >> 8) & 0xFFUL);
    osmgaD2Settle();
    IOLog("OpenStepMGA M4/%s: M1_4CB 8-2 measured u 127 far 127, v 128 far "
          "128, corner 127/127 on the trapezoid path; 0 and 255 would mean "
          "the neighbouring tap was clamped instead of wrapped\n", label);
    osmgaD2Settle();
}

/*
 * T8a.  The destination's top byte, which the repository already knows
 * arrives -- a texture whose top byte was 0xAB came back 0xAB when the
 * selector was wrong (:13249).  With the selector right it must carry the
 * INTERPOLATED alpha instead, which here ramps 0 .. 255 along dx.
 *
 * Reported as a residual range, not a pass: the ramp's rounding is not
 * something this project has measured, and a verdict that assumed it could
 * not discover it.
 */
static void
osmgaM4ReportAlpha(volatile unsigned long *blk, unsigned long stride,
                   unsigned long lo, unsigned long hi, const char *label,
                   unsigned long *outLo, unsigned long *outHi)
{
    unsigned long row, col, seen = 0UL, undrawn = 0UL;
    long dMin = 4096L, dMax = -4096L;
    unsigned long aLo = 256UL, aHi = 0UL;

    for (row = lo; row < hi; row++) {
        for (col = lo; col < hi; col++) {
            unsigned long v = blk[row * stride + col];
            unsigned long a, want;
            long d;
...
/*
 * T9's other half: run A drew with atype I, which writes no depth, so
 * NOTHING outside run B's triangle may hold a code.
 *
 * The whole block is scanned rather than a box around run A.  Run A's
 * triangles are scattered now, so a box would have to be maintained
 * alongside them -- and a leak is a leak wherever it lands, so the
 * cheaper and stricter question is "did depth appear anywhere it should
 * not".
 */
static unsigned long
...
{
    volatile unsigned short *z = (volatile unsigned short *)blkZ;
    unsigned long row, col, wrote = 0UL, maxv = 0UL;
    unsigned long badRow = 0UL, badCol = 0UL;

    for (row = 0UL; row < OSMGA_M3_BLK; row++)
        for (col = 0UL; col < OSMGA_M3_BLK; col++) {
            unsigned long v;

            /* run B's triangle is the one place depth belongs. */
            if (row >= OSMGA_M4_T9_BROW &&
                row <  OSMGA_M4_T9_BROW + OSMGA_M4_T9_BLEG &&
                col >= OSMGA_M4_T9_BCOL &&
                col <  OSMGA_M4_T9_BCOL + OSMGA_M4_T9_BLEG &&
                (row - OSMGA_M4_T9_BROW) + (col - OSMGA_M4_T9_BCOL) <=
                    OSMGA_M4_T9_BLEG - 1UL)
                continue;
            v = (unsigned long)z[row * stride + col];
            if (v != 0UL) {
                if (wrote == 0UL) { badRow = row; badCol = col; }
                wrote++;
                if (v > maxv) maxv = v;
            }
        }
    IOLog("OpenStepMGA M4/%s: %lu px outside run B hold depth (max %lu)"
          "%s\n", label, wrote, maxv,
          (wrote != 0UL) ? " -- the ZI state reached a run it should not"
                         : "");
    osmgaD2Settle();
    if (wrote != 0UL)
        IOLog("OpenStepMGA M4/%s: first leak at (%lu,%lu)\n",
              label, badRow, badCol);
    return wrote;
}

/*
 * osmgaM4CompareARGB lived here: a full 32-bit comparator with a coverage
 * count and a check that the covered pixels had moved off the prefill.
 * It was written for T8b's A/B against the trapezoid path, and that A/B
 * turned out to be impossible -- the oracle draws with SOLID_FILL, whose
 * atype does not consult ALPHACTRL.  osmgaM4CheckBlend below carries the
 * same three conditions against a computed expectation instead.  Should a
 * blend-capable Gouraud oracle ever be written, git has this one.
 */

/*
 * T8b's checker, which replaced an A/B that could not work.
 *
 * The trapezoid oracle draws with SOLID_FILL, and that atype does not
 * consult ALPHACTRL: on 2026-08-29 it wrote the source colour unchanged
 * while WARP blended, and the band reported "the two paths blend
 * differently" when what differed was the oracle's ability to blend at
 * all.  Fixing the oracle would mean changing the function T0 and T6
 * depend on, and both of those pass.
 *
 * So the expectation is computed instead, and it can be: the same run
 * measured the arithmetic to be
 *
 *      round( (src*a + dst*(255-a)) / 255 )
 *
 * in all four channels INCLUDING alpha -- checked in python against
 * 40906050 from a prefill of 00204060 and a source of 80ff8040.  The
 * integer form below needs no FPU, which the kernel does not have.
 *
 * Three conditions, because the comparator it replaces carried two of
 * them and dropping either would let a band that drew nothing pass:
 * every covered pixel equals the expectation, every covered pixel moved
 * off the prefill, and every pixel outside the triangle is still prefill.
 */
static unsigned long
osmgaM4CheckBlend(volatile unsigned long *blk, unsigned long stride,
                  unsigned long src, unsigned long dst, const char *label)
{
    unsigned long row, col, seen = 0UL, wrong = 0UL, unmoved = 0UL;
    unsigned long spilled = 0UL, expect = 0UL, sh;
    unsigned long a = (src >> 24) & 0xFFUL;
    unsigned long badRow = 0UL, badCol = 0UL, badGot = 0UL;

    for (sh = 0UL; sh < 32UL; sh += 8UL) {
        unsigned long c = (src >> sh) & 0xFFUL;
        unsigned long d = (dst >> sh) & 0xFFUL;

        expect |= (((c * a + d * (255UL - a)) + 127UL) / 255UL) << sh;
    }
    ...
    for (row = 0UL; row < OSMGA_M3_BLK; row++) {
        for (col = 0UL; col < OSMGA_M3_BLK; col++) {
            ...
            if (v != expect) {
                if (wrong == 0UL) {
                    badRow = row; badCol = col; badGot = v;
                }
                wrong++;
            }
        }
    }
    IOLog("OpenStepMGA M4/%s: expect %08lx | %lu px, %lu wrong, %lu still "
          "prefill, %lu spilled outside\n",
          label, expect, seen, wrong, unmoved, spilled);
    osmgaD2Settle();
    if (wrong != 0UL)
        IOLog("OpenStepMGA M4/%s: first at (%lu,%lu) got %08lx\n",
              label, badRow, badCol, badGot);
...
static void
osmgaM4Prefill(volatile unsigned long *blk, unsigned long stride,
               unsigned long value)
{
    unsigned long row, col;
...
- (void)runWarpTriangleVertexTest
{
    ...
    if (!osmgaDmaBuildTriangleList(ring, ringDwords,
                                   ...
                                   dwgctl, OSMGA_M3_ALPHACTRL_NOBLEND,
                                   0UL, 0UL,
                                   (const OSMGAM3Tex *)0,
                                   &tailDwords, &totalDwords)) {
...
- (void)runWarpDepthQualificationTest
{
    ...
    unsigned long m4z[3], m4rhw[3], m4tu[3], m4tv[3], m4corner[4];
    unsigned long m4wrapBad[3];
    unsigned long m4aLo[2], m4aHi[2];
    unsigned long m4t9Leak[2], m4t9Busy[2];
    ...
    if (osmgaWindowState != OSMGA_WINDOW_OPEN ||
        ...
        OSMGA_M12_TEX + OSMGA_M12_BYTES > osmgaMmapWindowEnd ||
    ...
    if (wrong == 0UL && differ == 0UL) {
      ...
      if (osmgaWarpQualBands != OSMGA_WARPQUAL_REPAIR) {
      ...
      if (osmgaWarpQualBands != OSMGA_WARPQUAL_SETTLED) {
         ...
         * clamp bits change, and only one axis sweeps at a time: a band
         * that wrapped u while leaving v clamped would pass a test that
         * looked at u alone, and the first draft did exactly that.
         *
         * The sweep is four texels a pixel over three whole periods, so a
         * wrapping map and a clamping map disagree on 33 of the 49 columns
         * (python).  Nothing subtle is being asked of the readback.
         */
        osmgaM4FillPlain(blkM4 + OSMGA_M4_REP_BASE / 4UL,
                         OSMGA_M4_REP_PITCH, OSMGA_M4_REP_ROWS, 0UL);
        m4tex.enable  = 1UL;
        ...
        for (i = 0UL; i < 3UL; i++) {
            const char *lbl = (i == 0UL) ? "T7a-clamp"
                            : (i == 1UL) ? "T7b-repeatU" : "T7c-repeatV";
            int sweepU = (i != 2UL), sweepV = (i == 2UL);
            int wrapU  = (i == 1UL), wrapV  = (i == 2UL);
            unsigned long clean = 0UL, worst = 0UL;

            m4tex.clamp = (i == 0UL) ? MGA_TEXCTL_CLAMPUV
                        : (i == 1UL) ? MGA_TEXCTL_CLAMPV
                                     : MGA_TEXCTL_CLAMPU;
            /* One state list for the axis; only the coordinates move
             * between phases, which is how T3 ran its four. */
            if (!osmgaM3StateRun(base, ring, ringDwords, ringPhys,
                                 osmgaWarpPipeHeld[OSMGA_D2C_PIPE], stride,
                                 (dwgctlFlat & ~MGA_DWGCTL_OPCODE_MASK) |
                                     MGA_DWGCTL_TEXTURE_TRAP |
                                     MGA_DWGCTL_NOZCMP,
                                 OSMGA_M3_ALPHACTRL_NOBLEND,
                                 OSMGA_M3_WARP, OSMGA_M3_ZORG, &m4tex, lbl)) {
                IOLog("OpenStepMGA M3: RETAINED, engine claimed.  *** "
                      "REBOOT REQUIRED ***\n");
                goto unmapM4;
            }
            for (ph = 0UL; ph < OSMGA_M4_REP_PHASES; ph++) {
                unsigned long bad;

                for (row = 0UL; row < OSMGA_M3_BLK; row++)
                    for (col = 0UL; col < OSMGA_M3_BLK; col++)
                        blkWa[row * stride + col] = OSMGA_S1_SENTINEL;
                m4corner[0] = m4corner[1] = m4corner[2] = m4corner[3] =
                    OSMGA_M3_COLOUR;
                if (sweepU)
                    osmgaM4SquareVertsUV(vtx, osmgaM4RepULo[ph],
                                         osmgaM4RepUHi[ph],
                                         OSMGA_M4_REP_HOLD,
                                         OSMGA_M4_REP_HOLD, m4corner);
                else
                    osmgaM4SquareVertsUV(vtx, OSMGA_M4_REP_HOLD,
                                         OSMGA_M4_REP_HOLD,
                                         osmgaM4RepULo[ph],
                                         osmgaM4RepUHi[ph], m4corner);
                if (!osmgaD2eSubmitBatch(base, vtx, vtxPhys,
                                         2UL * OSMGA_D2C_VTX_DWORDS, 1,
                                         lbl, 0)) {
                    IOLog("OpenStepMGA M3: RETAINED, engine claimed.  *** "
                          "REBOOT REQUIRED ***\n");
                    goto unmapM4;
                }
                bad = osmgaM4ReportWrap(blkWa, stride, OSMGA_M4_SQ_LO,
                                        OSMGA_M4_SQ_HI, sweepU, wrapU,
                                        sweepV, wrapV, lbl, 0);
                if (bad == 0UL) clean++;
        ...
        /*
         * T3 found one of its four phases sitting on a texel boundary and
         * wrong on 1175 of 1176 pixels while the other three were exact.
         * So THREE clean phases is the pass, not four: a boundary phase is
         * the anchor being where T3 measured it, not a defect here.
         */
        if (m4wrapBad[0] <= 1UL && m4wrapBad[1] <= 1UL &&
            m4wrapBad[2] <= 1UL)
            IOLog("OpenStepMGA M4: T7 PASS -- WARP-driven coordinates clamp "
                  "when clamped and wrap per axis when not, at one texel a "
                  "pixel\n");
        else
            IOLog("OpenStepMGA M4: T7 -- phases failing: clamp %lu, repeatU "
         ...
         *
         * Sixteen texels over two periods across 48 pixels magnifies, so
         * magfilter runs; u = 0 lands on dx = 3 and u = 1 on dx = 27, both
         * pixel centres, which is where a seam can be read at all.
         */
        osmgaM4FillSeam(blkM4 + OSMGA_M4_SEAM_BASE / 4UL,
                        OSMGA_M4_SEAM_PITCH, OSMGA_M4_SEAM_ROWS,
                        OSMGA_M4_SEAM_DIM);
        m4tex.enable  = 1UL;
        ...
        if (!osmgaM3StateRun(base, ring, ringDwords, ringPhys,
                             osmgaWarpPipeHeld[OSMGA_D2C_PIPE], stride,
                             (dwgctlFlat & ~MGA_DWGCTL_OPCODE_MASK) |
                                 MGA_DWGCTL_TEXTURE_TRAP | MGA_DWGCTL_NOZCMP,
                             OSMGA_M3_ALPHACTRL_NOBLEND,
                             OSMGA_M3_WARP, OSMGA_M3_ZORG, &m4tex, "T7e")) {
            IOLog("OpenStepMGA M3: RETAINED, engine claimed.  *** REBOOT "
                  "REQUIRED ***\n");
            goto unmapM4;
        }
        m4corner[0] = m4corner[1] = m4corner[2] = m4corner[3] =
            OSMGA_M3_COLOUR;
        osmgaM4SquareVertsUV(vtx, OSMGA_M4_SEAM_LO, OSMGA_M4_SEAM_HI,
                             OSMGA_M4_SEAM_LO, OSMGA_M4_SEAM_HI, m4corner);
        if (!osmgaD2eSubmitBatch(base, vtx, vtxPhys,
                                 2UL * OSMGA_D2C_VTX_DWORDS, 1, "T7e", 0)) {
            IOLog("OpenStepMGA M3: RETAINED, engine claimed.  *** REBOOT "
                  "REQUIRED ***\n");
            goto unmapM4;
        }
         ...
         *
         * TDUALSTAGE's alpha selector at nought means the TEXTURE's alpha
         * (:670); ARG2 with the rest of the register clear means DIFFUSE,
         * the interpolated one.  BOTH LANES take the same value, because
         * they are the even and odd screen columns and setting one alone
         * leaves a row reading `20 ab 28 ab` (:13249).
         *
         * The texture's own alpha is 0x33 so that the wrong answer is not
         * merely wrong but recognisable.  The second band is the negative
         * control: with the selector back at nought, 0x33 must come back.
         */
        osmgaM4FillPlain(blkM4 + OSMGA_M4_A8_BASE / 4UL,
                         OSMGA_M4_A8_PITCH, OSMGA_M4_A8_ROWS,
                         OSMGA_M4_TEX_ALPHA);
        m4tex.enable  = 1UL;
        ...
        for (i = 0UL; i < 2UL; i++) {
            const char *lbl = (i == 0UL) ? "T8a-diffuse" : "T8a-fromtex";

            m4tex.tds = (i == 0UL) ? MGA_TDS_ALPHA_SEL_ARG2 : 0UL;
            for (row = 0UL; row < OSMGA_M3_BLK; row++)
                for (col = 0UL; col < OSMGA_M3_BLK; col++)
                    blkWa[row * stride + col] = OSMGA_S1_SENTINEL;
            if (!osmgaM3StateRun(base, ring, ringDwords, ringPhys,
                                 ...
                                 (dwgctlFlat & ~MGA_DWGCTL_OPCODE_MASK) |
                                     MGA_DWGCTL_TEXTURE_TRAP |
                                     MGA_DWGCTL_NOZCMP,
                                 OSMGA_M4_ALPHACTRL_ACHAN,
                                 OSMGA_M3_WARP, OSMGA_M3_ZORG, &m4tex, lbl)) {
                IOLog("OpenStepMGA M3: RETAINED, engine claimed.  *** "
                      "REBOOT REQUIRED ***\n");
                goto unmapM4;
            }
            /* alpha 0 at the low column, 255 at the high one, so the ramp
             * runs along dx and both triangles agree on the diagonal. */
            m4corner[0] = 0x00FF8040UL;
            m4corner[1] = 0xFFFF8040UL;
            m4corner[2] = 0x00FF8040UL;
            m4corner[3] = 0xFFFF8040UL;
            osmgaM4SquareVertsUV(vtx, OSMGA_M4_REP_HOLD, OSMGA_M4_REP_HOLD,
                                 OSMGA_M4_REP_HOLD, OSMGA_M4_REP_HOLD,
                                 m4corner);
            if (!osmgaD2eSubmitBatch(base, vtx, vtxPhys,
                                     2UL * OSMGA_D2C_VTX_DWORDS, 1,
                                     lbl, 0)) {
            ...
            osmgaM4ReportAlpha(blkWa, stride, OSMGA_M4_SQ_LO,
                               OSMGA_M4_SQ_HI, lbl,
                               &m4aLo[i], &m4aHi[i]);
        }
        /*
         * The verdict, which this band did not have on 2026-08-29 and so
         * could only be read by eye -- and was read wrong.
         *
         * NOT EXERCISED comes first: if the two bands agree, the selector
         * never mattered and neither band measured it, whatever the values
         * happen to be.  That is exactly what happened when ALPHACTRL was
         * writing FCOL's alpha in both.
         */
        if (m4aLo[0] == m4aLo[1] && m4aHi[0] == m4aHi[1])
            IOLog("OpenStepMGA M4: T8a NOT EXERCISED -- both bands read "
                  "%lu..%lu, so TDUALSTAGE's selector changed nothing\n",
                  m4aLo[0], m4aHi[0]);
        else if (m4aHi[0] - m4aLo[0] >= 200UL &&
                 m4aLo[1] == OSMGA_M4_TEX_ALPHA &&
                 m4aHi[1] == OSMGA_M4_TEX_ALPHA)
            IOLog("OpenStepMGA M4: T8a PASS -- ARG2 delivers the "
                  "interpolated alpha (%lu..%lu) and ARG1 the texture's "
                  "(%lu)\n", m4aLo[0], m4aHi[0], OSMGA_M4_TEX_ALPHA);
        else
            IOLog("OpenStepMGA M4: T8a -- diffuse %lu..%lu (want a wide "
         ...
         *
         * A/B against the trapezoid path with the SAME ALPHACTRL, into a
         * destination both paths find already painted.  The blend
         * arithmetic itself is not on trial: M1_4E3 matched Mesa on nine
         * factor pairs across four channels with worst difference nought.
         *
         * The comparator is not osmgaM3CompareSurfaces.  That one masks
         * the alpha off and tests equality alone, so two surfaces left at
         * the prefill compare equal -- a band where NEITHER path drew would
         * have read as agreement.
         */
        osmgaM4Prefill(blkWa, stride, OSMGA_M4_PREFILL);
        if (!osmgaM3StateRun(base, ring, ringDwords, ringPhys,
                             osmgaWarpPipeHeld[OSMGA_D2C_PIPE], stride,
                             dwgctlFlat, OSMGA_M4_ALPHACTRL_OVER,
                             OSMGA_M3_WARP, OSMGA_M3_ZORG,
                             (const OSMGAM3Tex *)0, "T8b")) {
            IOLog("OpenStepMGA M3: RETAINED, engine claimed.  *** REBOOT "
                  "REQUIRED ***\n");
            goto unmapM4;
        }
        osmgaM3BuildTri(vtx, OSMGA_M3_TRI_LO, OSMGA_M3_TRI_LO,
                        OSMGA_M3_TRI_HI - OSMGA_M3_TRI_LO,
                        OSMGA_M3_F32_HALF, OSMGA_M4_SRC_COLOUR);
        if (!osmgaD2eSubmitBatch(base, vtx, vtxPhys, OSMGA_D2C_VTX_DWORDS,
                                 1, "T8b", 0)) {
            IOLog("OpenStepMGA M3: RETAINED, engine claimed.  *** REBOOT "
                  "REQUIRED ***\n");
        ...
        IOLog("OpenStepMGA M4/T8b: prefill %08lx, source %08lx, WARP %08lx "
              "at (%lu,%lu)\n", OSMGA_M4_PREFILL, OSMGA_M4_SRC_COLOUR,
              blkWa[(OSMGA_M3_TRI_LO + 4UL) * stride + OSMGA_M3_TRI_LO + 4UL],
              OSMGA_M3_TRI_LO + 4UL, OSMGA_M3_TRI_LO + 4UL);
        osmgaD2Settle();
        m4diff = osmgaM4CheckBlend(blkWa, stride, OSMGA_M4_SRC_COLOUR,
                                   OSMGA_M4_PREFILL, "T8b");
        if (m4diff == 0UL)
            IOLog("OpenStepMGA M4: T8b PASS -- WARP blends to the rounded "
                  "/255 result in all four channels, alpha included, and "
                  "nothing outside the triangle moved\n");
        else
      ...
      if (osmgaWarpQualBands != OSMGA_WARPQUAL_SETTLED) {
        ...
        for (i = 0UL; i < 2UL; i++) {
            ...
            osmgaM3BuildTri(vtx, OSMGA_M4_T9_BROW, OSMGA_M4_T9_BCOL,
                            OSMGA_M4_T9_BLEG, OSMGA_M3_F32_3Q,
                            OSMGA_M3_COLOUR);
            if (!osmgaD2eSubmitBatch(base, vtx, vtxPhys,
            ...
            osmgaM3ReportDepth(blkZ, stride, OSMGA_M4_T9_BROW,
                               OSMGA_M4_T9_BCOL, OSMGA_M4_T9_BLEG,
                               OSMGA_M3_Z_3Q, 0UL, lbl);
            m4t9Leak[i] = osmgaM4ReportRunA(blkZ, stride, lbl);
            m4t9Busy[i] = busyA;
            osmgaD2Settle();
        }
        /*
         * The verdict, which the first version left to the reader -- and
         * the reader (me) got it wrong, reading a 0/1 flag as a status
         * word and calling the band inconclusive when it had answered.
         *
         * NOT EXERCISED first: without drawing in flight at run A's
         * pointer boundary the unfenced band never put a state list on a
         * busy engine, and the two bands agreeing means nothing.
         */
        if (m4t9Busy[1] == 0UL)
            IOLog("OpenStepMGA M4: T9 NOT EXERCISED -- run A was already "
                  "done at the pointer boundary, so the unfenced band did "
                  "not test anything\n");
        else if (m4t9Leak[0] == 0UL && m4t9Leak[1] == 0UL)
            IOLog("OpenStepMGA M4: T9 PASS -- the second state list took "
                  "effect without leaking into a run still drawing, fenced "
                  "or not.  The per-run fence is not needed\n");
        else
       ...
       */
      {
        ...
        /* ---- M12: does the WARP path fetch mip levels? ---------------
         * M1-4D9 measured the trapezoid path never reading below the
         * base level and left "it lives on the WARP side" untested; the
         * old DRI ships a five-level WARP mip chain, so the question is
         * open exactly here.  Three-level atlas, level markers in alpha,
         * address signatures in RGB; two rhw regimes; NRST and BILIN as
         * controls beside the four mip encodings, all with the same
         * mapnb and origins so the minfilter nibble is the only mover.
         * One line per mode and regime; the mapnb/fthres/avgstride
         * ladder waits for this first answer. */
        {
            vm_address_t aMip = 0;
            unsigned long lMip = 0UL;
            volatile unsigned long *blkMip = 0;
            unsigned long m12orgs[5];
            unsigned long m12tu[3], m12tv[3], m12rhw[3], m12z[3];
            unsigned long lvl, arm, mode, scale, leg, o;
            unsigned long anyDeep = 0UL;
            static const unsigned long m12min[6] = {
                0x0UL, 0x2UL, 0x8UL, 0x9UL, 0xAUL, 0xCUL };
            static const char *m12name[6] = {
                "nrst", "bilin", "mm1s", "mm2s", "mm4s", "mm8s" };
            static const unsigned long m12lvlOff[3] = {
                0UL, OSMGA_M12_L1_OFF, OSMGA_M12_L2_OFF };
            OSMGAM3Tex m12tex;

            r = osmgaMapUncachedBlock(frameBufferPhysical, OSMGA_M12_TEX,
                                      OSMGA_M12_TEX + OSMGA_M12_BYTES,
                                      &aMip, &lMip, &blkMip);
            if (r != IO_R_SUCCESS) {
                ...
                for (lvl = 0UL; lvl < 3UL; lvl++) {
                    ...
                    for (row = 0UL; row < dim; row++)
                        for (col = 0UL; col < dim; col++) {
                            ...
                            unsigned long gg = (off >> 8) & 255UL;

                            blkMip[bas + off] =
                                ((0x40UL + 0x40UL * lvl) << 24)
                                | (rr << 16) | (gg << 8)
                                | ((rr * rr + gg * gg) & 255UL);
                        }
                }
                for (o = OSMGA_M12_GUARD_OFF / 4UL;
                     o < OSMGA_M12_BYTES / 4UL; o++)
                    blkMip[o] = 0xEE0000EEUL;

                m12orgs[0] = OSMGA_M12_TEX;
                m12orgs[1] = OSMGA_M12_TEX + OSMGA_M12_L1_OFF;
                m12orgs[2] = OSMGA_M12_TEX + OSMGA_M12_L2_OFF;
                /* Valid, in-window, and never legitimately fetched at
                 * mapnb 2 -- if either shows up in a signature the
                 * chain walked further than it was told to. */
                m12orgs[3] = m12orgs[2];
                m12orgs[4] = m12orgs[2];

                m12tex.enable   = 1UL;
                m12tex.org      = OSMGA_M12_TEX;
                m12tex.dim      = OSMGA_M12_L0_DIM;
                m12tex.log2dim  = 6UL;
                m12tex.dimH     = OSMGA_M12_L0_DIM;
                m12tex.log2dimH = 6UL;
                ...
                for (arm = 0UL; arm < 2UL; arm++) {
                    for (mode = 0UL; mode < 6UL; mode++) {
                        unsigned long cl0 = 0UL, cl1 = 0UL, cl2 = 0UL;
                        unsigned long cmix = 0UL, cbad = 0UL;
                        ...
                        for (scale = 0UL; scale < 3UL; scale++) {
                            /* legs 32/16/8 sample the same s 0..1 span:
                             * 64 texels over the leg = minification of
                             * 2, 4 and 8. */
                            leg = OSMGA_M12_L0_DIM >> (scale + 1UL);
                        ...
                        if (cl1 != 0UL || cl2 != 0UL)
                            anyDeep = 1UL;
                        IOLog("OpenStepMGA M12 %s/%s: L0 %lu L1 %lu L2 "
                              "%lu mix %lu bad %lu\n",
                              (arm == 0UL) ? "aff" : "per",
                              m12name[mode], cl0, cl1, cl2, cmix, cbad);
                        osmgaD2Settle();
                    }
                }
                osmgaM12MipOrgs = 0;
                if (anyDeep != 0UL)
                    IOLog("OpenStepMGA M12: MIP FETCH OBSERVED on the "
                          "WARP path -- Phase B is live\n");
                else
                    IOLog("OpenStepMGA M12: no mip fetch observed in "
                          "this ladder; the sweep arms come next\n");
                osmgaD2Settle();
                IOUnmapPhysicalFromIOTask(aMip, lMip);
            }
        }
